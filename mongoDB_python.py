...
c2 = {"idcli":1,"nom":"karimi","prenom":"farid",
        "commandes":
        [
        {"idcmde":1,"datecmde": datetime(2000,5,5,10,5,6)},
        {"idcmde":2,"datecmde": datetime(1980,5,5,10,5,6)}
        ],
        "adresse":{"pays":"maroc","ville":"rabat"}
        }
...
for cli in liste[:]:
        print(cli)
#condition pour les clients dont le nom est différent de 'karimi' =>
#{"nom": { "$ne" : "karimi"}}

print("selection 2 :")
#$ne : not equal => famille : ($ne(not equal),$eq(equal),$gt(greater than),$gte(greater than equal),$lt(lesss than),$lte(less than equal))
#$nin : not in => famille : ($nin, $in(in)) 
#$exits : if 'true' => document contains the field (including the case where its value is null)
#$exists => famille(element operators) : ($type : format => { field: { $type: <BSON type> } }, list of types => ref : https://www.mongodb.com/docs/manual/reference/operator/query/type/) 
...
for doc in selection:
        print("x:",doc["x"],"y:",doc["y"],"somme:",doc["somme"],
                ...
                doc["division"],"modulo:",doc["modulo"])
